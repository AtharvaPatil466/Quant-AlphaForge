...
AlphaForge is a quantitative alpha research platform with eleven components:
...
11. **`alphaforge-prediction/`** — Kalshi favorite-longshot-bias (FLB) research stack on Kalshi binary event contracts. Substrate #10, **Phase 0 CERTIFIED; Phase 1 INCONCLUSIVE (underpowered, MVE-only) 2026-06-17.** First substrate where small capacity is the *edge*, not the handicap; the goal is a credible live track record, not fund-scale alpha. The free read-only Kalshi host yields only a recent, MVE-heavy (~292 volume-bearing) universe (design §16 ADDENDUM); available N sits **25–140× below the `binary_mde` detection floor**, so per §11 it routes to forward paper accumulation (Phase 2 harness built). All statistics via the canonical `afgauntlet` binary/calibration module; the Phase 1 core was adversarially audited 7/7 integrity-clean. Contract: `research/PREDICTION_MARKETS_DESIGN.md`; verdict: `research/PHASE1_VERDICT.md`. NOTE: this file's "Project Status" narrative below predates the 2026-06 `alphaforge-gauntlet` package, the MDE power calibration, and the microstructure Phase-0-break findings — treat the per-substrate docs as authoritative where they conflict.
...
| 10 | Prediction Markets (Kalshi FLB) | PHASE 1 INCONCLUSIVE | 2026-06-17 | Underpowered — free Kalshi host is recent/MVE-only (~292 contracts), N sits 25–140× below the MDE floor; routes to forward paper accumulation (Phase 2 harness built). First "small-is-the-edge" substrate. |
...
- **`env/`** — Gymnasium `TradingEnv`. 57-dim obs, 5 discrete actions. Continuous action space is not implemented. Dense reward shaping (rolling Sharpe delta + drawdown penalty + participation) plus Sharpe-based terminal reward. Curriculum scheduler ramps tx costs, leverage, stops, episode length. `env/real_data.py` sources aligned OHLCV from the shared parquet store — training/validation never touch the network.
- **`agents/`** — `BaseAgent` wraps an `ActorCriticNetwork` with multi-head attention over per-ticker features. Continuous action space is not implemented. `DQNHead`, `PPOTrainer` (GAE + clipped surrogate), `MAMLTrainer` (FOMAML), `EnsemblePolicy`, `ParetoFront`, `AgentPool`.
