...
**`env/`** — Gymnasium-compatible `TradingEnv`. 57-dim float32 observations. Discrete action space (5 actions). Continuous action space is not implemented. Dense reward shaping: rolling Sharpe delta + drawdown-aware penalty + participation incentive per step, plus Sharpe-based episode-end reward. Curriculum learning progressively increases difficulty (tx costs, leverage limits, stop-loss tightness, episode length).

**`agents/`** — `BaseAgent` wraps an `ActorCriticNetwork` (shared trunk with multi-head attention over per-ticker features → actor/critic heads) and optional `DQNHead`. Continuous action space is not implemented. `PPOTrainer` runs GAE + clipped surrogate updates. `MAMLTrainer` provides FOMAML meta-learning for fast regime adaptation. `EnsemblePolicy` blends multiple agents' action distributions weighted by regime bandit. `ParetoFront` maintains non-dominated agents across multiple objectives. `AgentPool` manages population with ranked fitness, elite/survivor selection.
